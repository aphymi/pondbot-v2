...
def disable(*cmds):
	...
	# Disable individual commands. They might be given by alias, so we need to
	# resolve those.
	cmd_names = []
	for cmd in cmds:
...
def enable(*cmds):
	...
	Has the side effect/feature of immediately finishing the cooldown of
	included cmds.
	"""
	
	# Don't need to check whether c is cooled down, because removing it won't
	# have any affect anything even if it is.
	disabled_cmds = filter(
		lambda c: c.startswith("cmd."),
		# Accessing private vars is sketchy, but it's useful here.
		cooldown_list.keys()
	)
